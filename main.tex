...
\begin{uncoverenv}<3->
\begin{tikzpicture}[>=latex]
\node[rectangle callout, draw, inner sep=5 pt, callout relative pointer={(200:1 cm)}, text width=0.65\linewidth] {Why? Couldn't you just use XRootD to manage (move, backup, cache) columns directly? Why does it matter that they're inside of files?};
\end{tikzpicture}
\end{uncoverenv}
...
\vspace{0.5 cm}
Painful decisions need to be made to keep size down to 1--2~kB/event, to make data access easier for 50\% of analyses while completely excluding others.

\vspace{0.5 cm}
\begin{columns}
\column{0.55\linewidth}
\begin{center}
\uncover<2->{If we could really manage data at the columnar level, we could just put the most frequently used 1--2~kB {\it of each event} in warm storage and the other 40~kB in colder storage.}
\end{center}
...
\begin{uncoverenv}<3->
\hfill \begin{tikzpicture}[>=latex]
\node[rectangle callout, draw, inner sep=5 pt, callout relative pointer={(-200:-1 cm)}, text width=0.65\linewidth] {Because only ROOT knows how to interpret a branch's relationship with other branches.};
\end{tikzpicture}
\end{uncoverenv}
\end{frame}
